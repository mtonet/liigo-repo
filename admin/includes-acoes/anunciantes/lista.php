...
if($enviopesq==""){
$lista="SELECT id_cod,nome,email from tbl_usuarios ORDER BY nome asc";
$query=$mysqli->query($lista);
$row=$query->num_rows;
}elseif($enviopesq=="s"){
$lista="SELECT id_cod,nome,email from tbl_usuarios WHERE nome like'%".$pesq."%' OR email like'%".$pesq."%' ORDER BY nome asc";
$query=$mysqli->query($lista);
$row=$query->num_rows;
}
