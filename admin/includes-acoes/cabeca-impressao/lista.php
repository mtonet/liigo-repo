...
if($enviopesq==""){
$lista="SELECT id_cod,nome,status from tbl_cabeca_impressao ORDER BY nome asc";
$query=$mysqli->query($lista);
$row=$query->num_rows;
}elseif($enviopesq=="s"){
$lista="SELECT id_cod,nome,status from tbl_cabeca_impressao WHERE nome like'%".$pesq."%' ORDER BY nome asc";
$query=$mysqli->query($lista);
$row=$query->num_rows;
}
...
//deleta
$sqld="DELETE FROM tbl_cabeca_impressao WHERE id_cod='".$area."'";
